...
const Analysis = ({ team }: AnalysisProp) => {
  ...
  const teamTotalStats =
    team.length > 0 && team.every((p) => p.stats?.length > 0)
      ? team[0].stats.map((stat) => ({
          stat: statLabels[stat.name] || stat.name.toUpperCase(),
          value: team.reduce(
            (sum, p) =>
              sum + (p.stats.find((s) => s.name === stat.name)?.value || 0),
            0,
          ),
        }))
      : [];
  ...
  return (
    <div className="analysis-section">
      ...
      {Object.entries(weaknesses)
        .filter(
          ([type, pokemons]) =>
            pokemons.length >= 1 && resistances[type].length < 1,
        ) // Only show weaknesses that aren't fully resisted
        .sort((a, b) => b[1].length - a[1].length)
        .map(([type, pokemons]) => (
          <div
            key={type}
            className={`weakness-row ${pokemons.length >= 2 ? 'danger' : ''}`}
          >
            <span className={`type-badge ${type}`}>{type}</span>
            <div className="weak-pokemon-sprites">
              {pokemons.map((name) => (
                <span key={name} className="mini-name">
                  {name}
                </span>
              ))}
            </div>
