...
function App() {
  ...
  const[pokemonCache, setPokemonCache] = useState<Record<string, PokemonDetails>>({});
  ...
  const addToTeam = async (name: string) => {
    if (team.length >= 6) return alert("Team is full! Remove a Pokemon before adding another.");
    if (team.find(p => p.name === name)) return alert("Pokemon is already in your team!");
  ...
  return (
      <div className="app">
          ...
          <TeamDisplay team={team} onRemove={removeFromTeam} onEmptyClick={focusSearch} />
