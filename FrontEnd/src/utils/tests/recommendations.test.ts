...
const makePokemon = (name: string, types: string[], stats: Record<string, number> = {}) => ({
    name,
    types,
    sprite: '',
    id: 1,
...
test('warns when no physical attacker', () => {
    const team = [
        makePokemon('blissey', ['normal'], {
            ...
            'special-attack': 75, 'special-defense': 135, speed: 55
        })
    ];
    const recs = getRecommendations(team);
...
test('well balanced team returns positive message', () => {
    const team = [
        ...
        makePokemon('rotom', ['electric', 'water'], {
            ...
            'special-attack': 105, 'special-defense': 107, speed: 86
        }),
