...
export type Role = 'Physical Attacker' | 'Special Attacker' | 'Physical Tank' | 'Special Tank' | 'Fast Sweeper' | 'Balanced';

export const getRole = (pokemon: PokemonDetails): Role => {
    const getStat = (name: string) => pokemon.stats.find(s => s.name === name)?.value || 0;
