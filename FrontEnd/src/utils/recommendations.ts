...
export const getRecommendations = (team: PokemonDetails[]): string[] => {
  ...
  const hasPhysicalAttacker = roleNames.includes('Physical Attacker') || roleNames.includes('Fast Sweeper');
  const hasSpecialAttacker = roleNames.includes('Special Attacker');
  const hasTank = roleNames.includes('Physical Tank') || roleNames.includes('Special Tank');
  const hasFastSweeper = roleNames.includes('Fast Sweeper');

  if (!hasPhysicalAttacker) {
    recommendations.push('Your team lacks a Physical Attacker. Consider adding one for better damage output.');
  }
  if (!hasSpecialAttacker) {
    recommendations.push('Your team lacks a Special Attacker. Opponents with high Defense will be hard to beat.');
  }
  if (!hasTank) {
    recommendations.push('Your team has no defensive Pokemon. Consider adding a tank to absorb hits.');
  }
  if (!hasFastSweeper) {
    recommendations.push('Your team lacks a fast sweeper. You may struggle to outspeed opponents.');
  }

  // Check team size
  if (team.length < 6) {
    recommendations.push(`Your team only has ${team.length} Pokemon. Fill all 6 slots for a complete team.`);
  }
